...
public class AccessLevelTests
{
    private static readonly Dictionary<(string Method, string Path), AccessLevel> Intended = new()
    {
        ...
        // the rest of the admin-only channels card: the import takes any URL,
        // so at Edit it was POST /api/channels by another name (audit [61])
        [("POST", "/api/channels/import")] = AccessLevel.Admin,
        [("POST", "/api/channels/restart")] = AccessLevel.Admin,
        [("POST", "/api/channels/start")] = AccessLevel.Admin,
        [("POST", "/api/channels/stop")] = AccessLevel.Admin,
